...
<?php 
    ...
    // Check if recipe_section exists and contains necessary data
    $product_name_loop = !empty($recipe_section['product_name_loop']) ? $recipe_section['product_name_loop'] : null;
    $product_image = !empty($recipe_section['image']) ? $recipe_section['image'] : null;
    $product_title = !empty($recipe_section['title']) ? $recipe_section['title'] : null;
    $product_text = !empty($recipe_section['recipe_content']) ? $recipe_section['recipe_content'] : null;
	$product_background_color = $recipe_section['select_background_color'];
	
    // If any of the necessary fields are missing, do not display the section
    if ($recipe_section && ($product_name_loop || $product_image || $product_title || $product_text)) :
?>

<section class="inner-product-sec <?php echo 'background_color_',$product_background_color;?>">
    <div class="marquee a-section-marquee-box">
        <?php 
            if ($product_name_loop) :
...
<?php endif; ?>
	...
	<section class="top-recommendations" <?php if (has_term('combos', 'product_cat')) { echo 'style="display:none;"'; } ?>>
...
<?php if ( is_product() && has_term( 'combos', 'product_cat' ) ): ?>
	<section class="promo-section">
		<div class="container">
			<div class="left-div">
				<?php 
					// Get the left image URL from ACF options page
					$left_image = get_field('left_image', 'option');
					if ($left_image): ?>
						<img src="<?php echo esc_url($left_image); ?>" alt="Left Image">
					<?php endif; ?>
			</div>
			<div class="middle-div">
				<?php 
					// Get the heading text from ACF options page
				...
				<?php 
					// Get the body text from ACF options page
				...
				<?php 
					// Get the button URL from ACF options page
					$button_url = get_field('button_url', 'option');
					if ($button_url): ?>
						<a href="<?php echo esc_url($button_url); ?>" class="know-more-btn after-slides-btn">
							Place Bulk Order
						</a>
					<?php endif; ?>
			</div>
			<div class="right-div">
				<?php 
					// Get the right image URL from ACF options page
					$right_image = get_field('right_image', 'option');
					if ($right_image): ?>
						<img src="<?php echo esc_url($right_image); ?>" alt="Right Image">
					<?php endif; ?>
			</div>
		</div>
	</section>
